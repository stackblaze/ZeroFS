...
use crate::fs::inode::{ChunkHash, InodeId};
...
impl ChunkStore {
    ...
    /// Copy all chunks from source inode to destination inode (for COW snapshots)
    /// 
    /// This creates a shallow copy of chunk metadata - the actual chunk data is shared
    /// until one of the inodes modifies it. This enables instant snapshots without
    /// duplicating data.
    /// 
    /// Note: This is a workaround for the current architecture where chunks are keyed
    /// by (inode_id, chunk_index). A better long-term solution would be content-addressable
    /// storage where chunks are keyed by hash.
    pub async fn copy_chunks_for_cow(
        &self,
        source_id: InodeId,
        dest_id: InodeId,
        file_size: u64,
        ...
        let num_chunks = file_size.div_ceil(CHUNK_SIZE as u64);
        
        tracing::debug!(
            "Copying {} chunks from inode {} to inode {} (file_size={})",
            num_chunks,
            source_id,
            dest_id,
            file_size
        ...
        // Scan all chunks for the source inode
        let start_key = KeyCodec::chunk_key(source_id, 0);
        let end_key = KeyCodec::chunk_key(source_id, num_chunks);

        let mut stream = self.db.scan(start_key..end_key).await.map_err(|e| {
            error!("Failed to scan chunks for inode {}: {}", source_id, e);
            FsError::IoError
        })?;
        ...
        while let Some(result) = stream.next().await {
            let (key, value) = result.map_err(|e| {
                error!("Failed to read chunk during COW copy: {}", e);
                FsError::IoError
            })?;
            
            if let Some(chunk_idx) = KeyCodec::parse_chunk_key(&key) {
                // Create new chunk key with destination inode ID
                let dest_key = KeyCodec::chunk_key(dest_id, chunk_idx);
                
                // Copy the chunk data to the new key
                self.db
                    .put_with_options(
                        &dest_key,
                        &value,
                        ...
                        &slatedb::config::WriteOptions {
                            await_durable: false, // Batch for performance
                        },
                    )
                    .await
                    .map_err(|e| {
                        error!("Failed to copy chunk {} from inode {} to {}: {}", 
                            chunk_idx, source_id, dest_id, e);
                        FsError::IoError
                    })?;
        ...
        tracing::info!(
            "Copied {} chunks from inode {} to inode {} for COW",
            chunks_copied,
            source_id,
            dest_id
        );
    ...
    /// Store a chunk by its content hash (CAS)
    /// Returns the BLAKE3 hash of the chunk
    pub async fn put_by_hash(&self, data: &[u8]) -> Result<ChunkHash, FsError> {
        // Compute BLAKE3 hash
        let hash = blake3::hash(data);
        let hash_bytes: ChunkHash = *hash.as_bytes();
        
        // Check if chunk already exists (deduplication)
        let key = KeyCodec::chunk_key_by_hash(&hash_bytes);
        match self.db.get_bytes(&key).await {
            Ok(Some(_)) => {
                tracing::info!("Chunk {} already exists (deduplicated)", hex::encode(&hash_bytes[..8]));
                return Ok(hash_bytes);
            }
            Ok(None) => {
                tracing::info!("Chunk {} not found (will store)", hex::encode(&hash_bytes[..8]));
            }
            Err(e) => {
                tracing::warn!("Error checking for existing chunk {}: {}", hex::encode(&hash_bytes[..8]), e);
            }
        }
        
        // Store chunk data
        tracing::info!("Storing new chunk {} ({} bytes)", hex::encode(&hash_bytes[..8]), data.len());
        self.db
            .put_with_options(
                &key,
                data,
                &slatedb::config::PutOptions::default(),
                &slatedb::config::WriteOptions {
                    await_durable: true,  // Make immediately visible for testing
                },
            )
            .await
            .map_err(|e| {
                error!("Failed to store chunk by hash: {}", e);
                FsError::IoError
            })?;
        
        tracing::info!("Successfully stored chunk {}", hex::encode(&hash_bytes[..8]));
        Ok(hash_bytes)
    }

    /// Retrieve a chunk by its content hash (CAS)
    pub async fn get_by_hash(&self, hash: &ChunkHash) -> Result<Bytes, FsError> {
        let key = KeyCodec::chunk_key_by_hash(hash);
        self.db
            .get_bytes(&key)
            .await
            .map_err(|e| {
                error!("Failed to get chunk by hash: {}", e);
                FsError::IoError
            })?
            .ok_or_else(|| {
                error!("Chunk not found for hash {}", hex::encode(&hash[..8]));
                FsError::NotFound
            })
    }

    /// Read file data using CAS (from chunk hashes in inode)
    pub async fn read_cas(&self, chunks: &[ChunkHash], offset: u64, length: u64) -> Result<Bytes, FsError> {
        if length == 0 {
            return Ok(Bytes::new());
        }
        ...
        for chunk_idx in start_chunk..=end_chunk {
            // Get chunk data (or zeros if beyond file)
            let chunk_data = if chunk_idx < chunks.len() {
                self.get_by_hash(&chunks[chunk_idx]).await?
            } else {
                Bytes::from_static(ZERO_CHUNK)
            };
    ...
    /// Write file data using CAS (updates chunk hashes in inode)
    pub async fn write_cas(
        ...
        if chunks.len() <= end_chunk {
            chunks.resize(end_chunk + 1, [0u8; 32]);
        }
        ...
        for chunk_idx in start_chunk..=end_chunk {
            ...
            // Read existing chunk if we're doing partial write
            let will_overwrite_fully = offset <= chunk_start && end_offset >= chunk_end;
            let mut chunk_data = if will_overwrite_fully {
                vec![0u8; CHUNK_SIZE]
            } else if chunk_idx < chunks.len() && chunks[chunk_idx] != [0u8; 32] {
                self.get_by_hash(&chunks[chunk_idx]).await?.to_vec()
            } else {
                vec![0u8; CHUNK_SIZE]
            };
            ...
            // Store chunk and get its hash
            tracing::info!("About to call put_by_hash for chunk {} ({} bytes)", chunk_idx, chunk_data.len());
            let hash = self.put_by_hash(&chunk_data).await?;
            tracing::info!("put_by_hash returned hash {}", hex::encode(&hash[..8]));
            chunks[chunk_idx] = hash;
        }
